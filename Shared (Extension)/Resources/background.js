                ...
                if (text && text.trim() !== "") {
                    try {
                        ...
                        // 2. ACTIVE TRIGGER
                            ...
                            // Prevent routing if the payload was just the CPH empty signal
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    // Virtual Participation Fallback
    if (msg.type === 'cf_virtual_fallback') {
        const data = msg.payload;
        const contestMatch = data.url.match(/(contest|gym)\/(\d+)\/problem\/([A-Za-z0-9]+)/);
        if (contestMatch) {
            const fallbackUrl = 'https://codeforces.com/problemset/submit';
            data.problemCode = `${contestMatch[2]}${contestMatch[3]}`.toUpperCase();
    ...
    // MEMORY FLUSH: Instantly clear the lock after a success or a timeout.
    if (msg.type === 'cph_injection_failed' || msg.type === 'cph_injection_success') {
